...
function Chat({user}){
    ...
    const [usersConnectes, setUsersConnectes]=useState([])

useEffect(()=>{
    socketRef.current= io('http://localhost:3000')

    socketRef.current.emit('rejoindre', user.username)

    socketRef.current.on('usersConnectes', (users)=>{
        setUsersConnectes(users)   
    })
...
return(
    <div className="bg-gray-900 h-screen flex flex-col ">
        ...
        <div className="flex flex-1 overflow-hidden">
            <UserList users={usersConnectes} currentUser={user.username}/>
            <div className="flex-1 flex flex-col">
                <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-3">
                    ...
                    <div ref={bottomRef}>
                        {typing && (
                            <p className="text-violet-400 text-xs px-6 pb-1 italic animate-pulse">
                                ✍️ {typing} est en train d'écrire...
                            </p>
                        )}
                    </div>
                </div>
                    <div className="bg-gray-800 px-6 py-4 flex gap-3 border-t border-gray-700">
                        <input className="flex-1 bg-gray-900 text-white p-3 rounded-lg outline-none border border-gray-700"
